...
# V5.11.0: configured/manual watch items are NOT priority-scanned. They share
...
# V5.11.0: consecutive confirmations are capped and reset on a failed pass.
with tempfile.TemporaryDirectory() as d:
...
print('V5.11.0 BASE AUDIT TESTS OK')
...
# V5.11.0: Gecko 429 is not immediately retried; it enters a bounded cooldown.
class Resp:
...
# V5.11.0: normal technical path must not call Gecko /tokens/.../pools after a
...
# V5.11.0: invalid/missing Dex pair metadata may use Gecko pool discovery as fallback.
...
# V5.11.0: on-chain discovery must contribute tokens even when Dex feeds do not.
...
print('V5.11.0 FULL TESTS OK')

print('V5.11.0 FULL TESTS OK')

# V5.11.0: Blockscout token pagination is deliberately one page per scan; stale
...
# V5.11.0: capital allocation is explicit and risk-based. With $20, an 8% stop
...
# V5.11.0: opportunity alerts surface BUY CANDIDATE/WATCH ideas even when entry is
...
# V5.11.0: the deep budget reserves slots for newly created pairs so discovery
...
# V5.11.0: global Dex discovery accepts multiple major chains, not only Robinhood.
...
# V5.11.0: Gold is a first-class commodity radar asset and uses technical scoring,
...
# V5.11.0 runtime hardening: Bot never reuses a persisted Blockscout cursor across runs.
with tempfile.TemporaryDirectory() as d:
...
# V5.11.0: a failed primary Gecko pool is followed by one bounded fallback pool.
...
# V5.11.0: RETEST is not entry-ready until resistance is actually reclaimed.
...
print('V5.11.0 RUNTIME HARDENING TESTS OK')

print('V5.11.0 GLOBAL MULTI-ASSET TESTS OK')


# V5.11.0: Telegram discovery unwraps the (items, cursor) tuple and never iterates None.
with tempfile.TemporaryDirectory() as d:
    cp=Path(d)/'config.json'; sp=Path(d)/'state.json'; cp.write_text(json.dumps(cfg)); b=m.Bot(str(cp),str(sp))
    class TgAPI:
...
# V5.11.0: Telegram opportunity/risk views are available and main menu exposes them.
...
print('V5.11.0 TELEGRAM STRUCTURE TESTS OK')

# V5.11.0: the real GitHub Actions workflow path/version must match the code/config.
...
assert 'Global Multi-Asset Scanner V5.11.0' in wf
assert 'global-multi-asset-state-v5.11.0-' in wf
...
print('V5.11.0 WORKFLOW ALIGNMENT TESTS OK')
