...
The existing public runner workflow calls the private bot with `--once`. In V5.12.0 FINAL, `--once` also processes a bounded batch of Telegram updates after the scan. This makes Telegram usable without a permanent server; command response latency is normally up to one scheduled runner interval.
